...
Page({
  ...
  getPostList: function (cb) {
    ...
    console.log('开始获取诗歌列表，skip:', skip, 'page:', this.data.page);

    wx.cloud.callFunction({
      name: 'getPostList',
      data: { skip: skip, limit: PAGE_SIZE, isPoem: true }, // 只获取诗歌帖子
      success: res => {
        console.log('获取诗歌列表结果:', res);
        if (res.result && res.result.success) {
          const posts = res.result.posts || [];
          console.log('获取到诗歌数量:', posts.length);
          ...
          if (newPostList.length > 0) {
            ...
            if (currentBgImage) {
              console.log('设置背景图片:', currentBgImage);
              this.setData({
